...
public:
    ...
    */
    ...
    void useBuffer() {
        ...
        // 테스트용 : 별도 쓰레드에서 30ms마다 큐를 하나씩 꺼내서 printf (원래는 저장과 동일한 사이즈로 맞춰야함)
        while (m_endFlag != true)
        {
            std::string result;
            {
                ...
                if (!mp_currentBuffer->empty())
                {
                    std::string popData = mp_currentBuffer->front();
...
int main()
{
    ...
    replayer->loadFile("C:\\data\\scenario_1712130547987.dat");
